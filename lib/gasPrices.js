...
const GAS_COST = {
    ...
    ethTransfer: 21000, // native token
...
export async function getPolygonGasPrices() {
    ...
    return {
        low: ethers.utils.formatEther(parseInt(response.result.SafeGasPrice) * 10**8),
        medium: ethers.utils.formatEther(parseInt(response.result.ProposeGasPrice) * 10**8),
        high: ethers.utils.formatEther(parseInt(response.result.FastGasPrice) * 10**8)
    }
}
/*
...
getAverageGasCosts("erc20Transfer", "polygon").then(transactionCost => {
    console.log(`\nprice for an ERC20 transfer on polygon is ${transactionCost.MATIC} MATIC or ${transactionCost.USD} USD`);
});

getAverageGasCosts("ethTransfer", "polygon").then(transactionCost => {
    console.log(`\nprice for an MATIC transfer on polygon is ${transactionCost.MATIC} MATIC or ${transactionCost.USD} USD`);
});
